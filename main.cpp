...
int main() {
    ...
    while (running) {
        if (!shown) {
            ...
            Vec2<int> windowSize;
            auto window = SDL_CreateWindow("volund", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 600, 200,
            ...
            keyMaps.insert({SDLK_RETURN, [&]() {
                                searchText[0] = '\0';
                                if (topTen[0]) system(("setsid " + apps[*topTen[0]] + '&').c_str());
                                shown = false;
                                return true;
                            }});
            ...
            while (running && shown) {
                ...
                if (nk_begin(ctx, "volund", nk_rect(0, 0, windowSize.x, windowSize.y), 0)) {
                    ...
                    for (size_t i = 0; i < topTen.size() && i < 9; i++) {
                        ...
                        if (nk_button_label(ctx, (i == 0 ? (std::string{">>  "} + *topTen[i] + "  <<").c_str()
                                                         : topTen[i]->c_str()))) {
